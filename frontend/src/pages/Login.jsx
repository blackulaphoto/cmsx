...
function Login() {
  ...
  const { login, register, signInWithGoogle, configError } = useAuth()
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-md rounded-2xl border border-white/10 bg-slate-900 p-8 text-white shadow-2xl">
        ...
        {configError ? (
          <div className="mt-4 rounded-lg border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-200">
            {configError}. Add the Firebase web env vars in Vercel and redeploy.
          </div>
        ) : null}
        {mode === 'register' ? (
        ...
        <button className="mt-6 w-full rounded-lg bg-cyan-500 px-4 py-3 font-medium text-slate-950 disabled:opacity-50" type="submit" disabled={Boolean(configError)}>
          {mode === 'register' ? 'Register' : 'Login'}
        </button>
        <button className="mt-3 w-full rounded-lg border border-white/15 px-4 py-3 disabled:opacity-50" type="button" onClick={handleGoogle} disabled={Boolean(configError)}>
