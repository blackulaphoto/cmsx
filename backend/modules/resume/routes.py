    ...
    try:
        ...
        return {
            ...
            "profile": {
                ...
                "professional_references": profile.professional_references,
                "career_objective": profile.career_objective,
                "preferred_industries": profile.preferred_industries,
                "background_friendly_only": profile.background_friendly_only,
                "created_at": profile.created_at.isoformat() if hasattr(profile.created_at, 'isoformat') else str(profile.created_at) if profile.created_at else None,
                "updated_at": profile.updated_at.isoformat() if hasattr(profile.updated_at, 'isoformat') else str(profile.updated_at) if profile.updated_at else None
            }
        }
    ...
    try:
        ...
        for resume in resumes:
            ...
            resume_list.append({
                ...
                "created_at": resume.created_at.isoformat() if hasattr(resume.created_at, 'isoformat') else str(resume.created_at) if resume.created_at else None,
...
async def generate_resume_pdf(resume_id: str):
    ...
    try:
        ...
        success = generate_professional_pdf(resume, client, pdf_path)
...
def generate_professional_pdf(resume: Resume, client: Client, pdf_path: str) -> bool:
    """Generate professional PDF using WeasyPrint"""
    try:
        # Parse resume content
        resume_content = json.loads(resume.content) if isinstance(resume.content, str) else resume.content
        ...
        generated_path = generate_resume_pdf(resume_data, client_data, resume.template_type)
        
        if generated_path:
            # If the generated path is different from expected, copy/move the file
            if generated_path != pdf_path:
                import shutil
                shutil.copy2(generated_path, pdf_path)
            return True
        else:
            return False
